...
test_enable() {
  ...
  docker run --rm -e PHP_EXTENSION_BLACKFIRE=1 "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" \
    php -m | grep -q blackfire
  assert_equals "0" "$?"
}
test_alertConflictWithXDebug() {
  # Tests that blackfire + xdebug will output an error
  RESULT="$(docker run --rm -e PHP_EXTENSION_XDEBUG=1 -e PHP_EXTENSION_BLACKFIRE=1 \
