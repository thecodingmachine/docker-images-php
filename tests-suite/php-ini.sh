...
test_templateDefaultErrorReporting() {
  RESULT="$(docker run ${RUN_OPTIONS} --rm "${REPO}:${TAG_PREFIX}${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" php -i | tail -n +1 | grep error_reporting)"
...
test_templateProductionErrorReporting() {
  RESULT="$(docker run ${RUN_OPTIONS} --rm -e TEMPLATE_PHP_INI=production \
    "${REPO}:${TAG_PREFIX}${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" php -i | tail -n +1 | grep error_reporting)"
...
test_sessionSavePath() {
  RESULT="$(docker run ${RUN_OPTIONS} --rm -e PHP_INI_SESSION__SAVE_PATH="tcp://localhost?auth=yourverycomplex\"passwordhere" \
    "${REPO}:${TAG_PREFIX}${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" php -i | tail -n +1 | grep "session.save_path")"
  [[ "$RESULT" == "" ]]
...
test_smtp() {
  RESULT="$(docker run ${RUN_OPTIONS} --rm -e PHP_INI_SMTP="192.168.0.1" \
    "${REPO}:${TAG_PREFIX}${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" php -i | tail -n +1 | grep "^SMTP")"
...
test_disabledFunctionsIsCommented() {
  RESULT="$(docker run ${RUN_OPTIONS} --rm \
    "${REPO}:${TAG_PREFIX}${PHP_VERSION}-${BRANCH}-${BRANCH_VARIANT}" php -i | tail -n +1 | grep "disable_functions")"
