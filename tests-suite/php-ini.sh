...
test_templateProductionErrorReporting() {
  RESULT="$(docker run --rm -e TEMPLATE_PHP_INI=production \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep error_reporting)"
  assert_equals "error_reporting => 22527 => 22527" "$RESULT" "Wrong production error reporting"
}
test_templateCustomErrorReporting() {
  RESULT="$(docker run --rm -v "${SCRIPT_DIR}/assets/php-ini/php.ini:/etc/php/${PHP_VERSION}/cli/php.ini" \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep error_reporting)"
...
test_environmentErrorReporting() {
  RESULT="$(docker run --rm -e PHP_INI_ERROR_REPORTING="E_ERROR | E_WARNING" \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep error_reporting)"
...
test_sessionSavePath() {
  RESULT="$(docker run --rm -e PHP_INI_SESSION__SAVE_PATH="tcp://localhost?auth=yourverycomplex\"passwordhere" \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep "session.save_path")"
  [[ "$RESULT" == "" ]]
...
############################################################
test_smtp() {
  RESULT="$(docker run --rm -e PHP_INI_SMTP="192.168.0.1" \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep "^SMTP")"
...
test_disabledFunctionsIsCommented() {
  RESULT="$(docker run --rm \
    "thecodingmachine/php:${PHP_VERSION}-${BRANCH}-slim-${BRANCH_VARIANT}" php -i | grep "disable_functions")"
